    ...
    for (let workflow_name of workflows) {
      ...
      if (workflow_name in metadata_data["tasks"]) {
        if ("metadata" in metadata_data["tasks"][workflow_name]) {
          for (const var_val in metadata_data["tasks"][workflow_name][
            "metadata"
          ]["vars"]) {
            const var_object =
...
async function fetchViaWSL(
  ...
  return new Promise((resolve, reject) => {
    ...
    const cmd =
      `wsl bash -il -c "curl -s -L -w '__STATUS__%{http_code}' '${url}'"`;
...
connection.onDidChangeConfiguration((change) => {
  if (hasConfigurationCapability) {
    ...
    globalSettings = <ExampleSettings>(change.settings.dse || defaultSettings);
  }
  ...
  if (item.data == "channel_keyword") {
    ...
    if (workflow_obj["vars"].includes(item.label)) {
      ...
      if (value_suggestions === undefined) {
        ...
        item.insertText =
          "var " + item.label + " ${1|" + value_suggestions + "|}";
      }
    }
